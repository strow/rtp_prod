function airs_l1bqap_download(sdate, edate, asldata)
% function airs_l1bqap_download(sdate, edate, asldata)
% 
%   Downloads AIRS L1bQAP (QaSup) data tha fits in the requested date range.
%
...
% Breno Imbiriba - 2013.08.01
  ...
  for day = floor(sdate):floor(edate)
    ...
    for ig=g0:g1
      ...
      file_glob = [asldata '/AIRIBQAP/' syyy '/' sm '/' sd '/AIRS.' syyy '.' sm '.' sd '.' sgg '.L1B.AIRS_QaSub.*.hdf'];
      ddir = dir(file_glob);
      if(numel(ddir)==0)
	% File does not exist.
	% Issue download command:
	dnldcmd = ['$RTPROD/bin/getairs ' syyy sm sd ' 1 AIRIBQAP.005 ' sgg ' ' asldata ];
	disp(dnldcmd);

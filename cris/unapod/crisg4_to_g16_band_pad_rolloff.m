function [r1, r2, r3] = crisg4_to_g16_band_pad_rolloff(id, rad);

% function [r1, r2, r3] = crisg4_to_g16_band_pad_rolloff(id, rad);
%
% Split CrIS "g4" Hamming radiance spectra into bands and pad the
% channels to g16 using a combination of buddy channels and
% repeated channels, and then apply rolloff.
% The output radiance extends from 0 wavenumbers up to a few
% wavenumbers (16 channels) past the true upper band edge. 
% 
% Input:
...
%
% Output:
%    r1/r2/r3 - [1769/1417/1037 x nobs] padded and rolled off radiance
%

% Created: 07 July 2010, Scott Hannon
% Update: 09 Aug 2011, S.Hannon - create from "crisg4_to_g10_band_pad_rollof.m"
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Number of guard channels, buddy channels, non-guard buddy
% channels, and repeated channels at each edge of each band
nguard = 4;
nbuddy = 10;
...
for bnum = 1:3
   ...
   % Repeated radiance for edges
   % Note: it might be better to generate from BT but this is easier
   rrepeatlo = ones(nrepeat,1)*rb(1,:);
   rrepeathi = ones(nrepeat,1)*rb(2*nbuddy,:);
   ...
   ilos = round(nbout - nrepeat - 2*nbuddyonly - nbin + 1); % exact integer
   iloe = ilos + nbuddyonly - 1;
   ihis = nbout - nrepeat - nbuddyonly + 1;
   ihie = nbout - nrepeat;
   ind = ((iloe+1):(ihis-1));
   eval(['r' bstr '(ind,:) = rad(indg4,:);']);

   % Plug in lo edge buddy radiance and repeated radiance
   ind = ilos:iloe;
   indb = (1:nbuddyonly);
   eval(['r' bstr '(ind,:) = rb(indb,:);']);
   ind = ilos - 1 - nrepeat + (1:nrepeat);
   eval(['r' bstr '(ind,:) = rrepeatlo;']);

   % Plug in hi edge buddy radiance and repeated radiance
   ind = ihis:ihie;
   indb = ((nbuddy + nguard + 1):(2*nbuddy));
   eval(['r' bstr '(ind,:) = rb(indb,:);']);
   ind = ihie + (1:nrepeat);
   eval(['r' bstr '(ind,:) = rrepeathi;']);

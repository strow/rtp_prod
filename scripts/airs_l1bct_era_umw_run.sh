...
then
  ...
  bn=`basename $0`
  srun --partition=batch --cpus-per-task=1 --mem-per-cpu=4096 --ntasks=128 --job-name=CtTrkALL --qos=long_contrib --output=log/$bn-%j.%t.out $0 onnode &
...
then
  ...
  start_time='[2003,11,19,0,0,0]'
  #  end_time='[2002,09,01,0,59,59.999]'
    end_time='[2007,12,31,23,59,59.999]'

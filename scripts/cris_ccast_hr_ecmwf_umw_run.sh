...
then
  ...
  bn=`basename $0`
  srun --partition=batch --cpus-per-task=1 --ntasks=8 --mem-per-cpu=4096 --job-name=CrisHR --qos=long_contrib --output=log/$bn-%j.%t.out $0 onnode &
...
then
  ...
  matlab -nosplash -nodesktop -nodisplay -r "\
    timeblock_dealer($start_time, $end_time, $delta_time, $PE, $NPE, @cris_ccast_hr_ecmwf_umw ); \
    exit" 

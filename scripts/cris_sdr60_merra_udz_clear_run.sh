...
then
  ...
  bn=`basename $0`
  srun --partition=batch --cpus-per-task=1 --mem-per-cpu=4096 --ntasks=8 --job-name=CrsClrPrc --qos=long_contrib --output=log/$bn-%j.%t.out $0 onnode &
...
then
  ...
  /asl/opt/bin/matlab -nosplash -nodesktop -nodisplay -r "\
    timeblock_dealer($start_time, $end_time, $delta_time, $PE, $NPE, @cris_sdr60_merra_udz_clear ); \
    exit" 

...
then
  ...
  srun --partition=batch --cpus-per-task=2 --ntasks=24 --exclusive --job-name=CrsClrPrc --qos=long_contrib --output=slurm-%j.%t.out $0 onnode &
...
#    end_time='[2012, 09, 20,  0, 59,  59.999]'
  ...
  /asl/opt/bin/matlab -nosplash -nodesktop -nodisplay -r "\
    timeblock_dealer($start_time, $end_time, $delta_time, $PE, $NPE, @airs_l1bcm_proc ); \
    exit" 

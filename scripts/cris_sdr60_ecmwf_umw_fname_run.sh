...
# srun --ntasks=4 --job-name=test --qos=long_contrib --output=slurm-%j.%t.out cris_test_run.sh file1 ... fileN
#
...
then
  ...
  srun --partition=batch --cpus-per-task=1 --ntasks=1 --mem-per-cpu=4096 --job-name=CRIS_SDR --qos=long_contrib --output=log/$bn-%j.%t.out $0 onnode "$@" &
...
then
  ...
  #echo "test_cris_driver($start_time, $end_time, $delta_time, $PE, $NPE); exit"

  matlab -nosplash -nodesktop -nodisplay -r "\
  filename_dealer $PE  $NPE  $file_list  @cris_sdr60_ecmwf_umw_fname; \
  exit" 

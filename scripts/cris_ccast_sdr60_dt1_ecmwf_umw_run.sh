...
then
  ...
  bn=`basename $0`
  srun --partition=batch --cpus-per-task=1 --mem-per-cpu=4096 --ntasks=16 --job-name=sdr60dt1_ccast  --qos=long_contrib --output=log/$bn-%j.%t.out $0 onnode &
...
#    end_time='[2012, 09, 20,  0, 59,  59.999]'
  ...
  matlab -nosplash -nodesktop -nodisplay -r "\
    timeblock_dealer($start_time, $end_time, $delta_time, $PE, $NPE, @cris_ccast_sdr60_dt1_ecmwf_umw ); \
    exit" 

...
then
  ...
  bn=`basename $0`
  srun --partition=batch --cpus-per-task=1 --ntasks=1 --job-name=ArsL1bcmEcm --qos=long_contrib --output=log/slurm-$bn-%j.%t.out $0 onnode &

elif [ "$1" == 'onnode' ]
then

  echo on node...
  start_time='[2013, 09, 20, 05,  0,  0]'
    end_time='[2013, 09, 20, 05, 59, 59.999]'
  # 1-hr blocks  
  ...
  matlab -nosplash -nodesktop -nodisplay -r "\
    timeblock_dealer($start_time, $end_time, $delta_time, $PE, $NPE, @airs_l1bcm_era_umw ); \
    exit" 
  echo "Exited MATLAB"

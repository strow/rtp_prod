...
bn=basename(f{1});
outfile_cld = [dirname(f{1}) '/cld_era_38ch_cng10.' bn];
outfile_clr = [dirname(f{1}) '/clr_era_38ch_cng10.' bn];
...
prof.cngwat(ii) = prof.cngwat(ii) * 10;
%
iceflag = zeros(1,nprof);
ii = find(prof.ctype2 == 201);
iceflag(ii) = 1;
prof.cpsize2 = fake_cpsize(tavg2, iceflag, 1);
prof.cngwat2(ii) = prof.cngwat2(ii) * 10;

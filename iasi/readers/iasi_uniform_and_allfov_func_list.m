function [head,hattr,prof_out,pattr,s,isubset] = iasi_uniform_and_allfov_func_list(iasifile_list, allfov)
...
%
% Input:
%    iasifile_list  - Cell array of IASI granule files to read
...
% Renamed: 26 Jun 2013, B.Imbiriba - Using file list instead of glob
%    and subset list
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Set paths and assign constants

  rn='iasi_uniform_and_allfov_func_list';
  greetings(rn);
  ...
  iasifiles = iasifile_list;
  
  say(['Found ' int2str(length(iasifiles)) ' IASI files']);
  ...
  for ic = 1:length(iasifiles)
    ...
    say(['stime: ' datestr(datenum(2000,1,1,0,0,min(p.rtime)))]);
    say(['etime: ' datestr(datenum(2000,1,1,0,0,max(p.rtime)))]);

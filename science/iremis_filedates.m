...
%%iremis_files_ndate=datenum(iremis_files_date_str,'yyyy ddd');
iremis_files_ndate=datenum(iremis_files_date_str,'yyyy dd');

   
% $$$ flist=findfiles('/asl/data/iremis/CIMSS/iremis_*_idx.mat');
% $$$ regpat='_(20..)(...)_idx.mat';
% $$$ iremis_files_ndate=cellfun(@(x) datenum([x{1} ' ' x{2}],'yyyy dd'),cellfun(@(f) regexp(f, regpat,'tokens'),flist));
